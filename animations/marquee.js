...
const drawNameOnScreenRow = (text, x, select, glitch) => {
...
const drawNameOnScreenCol = (text, x, select, glitch) => {
  for (let index = 0; index < text.length; index++) {
...
const main = (args) => {
  ...
  setInterval(() => {
    ...
    for (const text of textData) {
      ...
      const drawFn = text.isVertical
        ? drawNameOnScreenCol
        : drawNameOnScreenRow;
      text.pos = ((text.pos + Math.floor(text.speed)) + offset) %
        moduloLength;
